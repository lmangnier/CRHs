...
PC.logoe.clipped.p = prcomp(log(oe.dense.clipped.p+0.0001))

jpeg(paste0("PCplots/",chr,"p_log_clipped_E1",resol[j],".jpg"))
plot(PC.logoe.clipped.p$rotation[,1],type="l",ylab="E1")
abline(h=0)
dev.off()
...
write.table(cbind(chr,pos.p,PC.logoe.clipped.p$rotation[,1:3]), paste0("PC_logoe/",chr,"p_",resol[j],"_PC_logoe_clipped.txt"), sep="\t",col.names=F,row.names=F,quote=F)
}
...
# Analyse en composantes principales
PC.logoe.clipped.q = prcomp(log(oe.dense.clipped.q+0.0001))

jpeg(paste0("PCplots/",chr,"q_log_clipped_E1",resol[j],".jpg"))
plot(PC.logoe.clipped.q$rotation[,1],type="l",ylab="E1")
abline(h=0)
dev.off()

#Exportation des 3 premieres composantes pour les analyses en lien avec la densite de genes pour choisir la 1CP (Gorkin et al., 2019)
write.table(cbind(chr,pos.q,PC.logoe.clipped.q$rotation[,1:3]), paste0("PC_logoe/",chr,"q_",resol[j],"_PC_logoe_clipped.txt"), sep="\t",col.names=F,row.names=F,quote=F)
}
}
}
# Ensuite: cat chr*_100Kb_PC_logoe_clipped.txt > allchrarms_PC_logoe_clipped.100Kb.txt
# cat chr*_500Kb_PC_logoe_clipped.txt > allchrarms_PC_logoe_clipped.500Kb.txt
