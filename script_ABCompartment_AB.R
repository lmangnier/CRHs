...
write.table(cbind(chr,pos.500Kb,PC.oe.500Kb$rotation[,1:3]), paste0("PC_oe/",chr,"_PC_oe.500Kb.txt"), sep="\t",col.names=F,row.names=F,quote=F)
write.table(cbind(chr,pos.1Mb,PC.oe.1Mb$rotation[,1:3]), paste0("PC_oe/",chr,"_PC_oe.1Mb.txt"), sep="\t",col.names=F,row.names=F,quote=F)
}

# Ensuite: cat chr*_PC_oe.500Kb.txt > allchrs_PC_oe.500Kb.txt
